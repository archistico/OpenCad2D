...
| v0.8.145 | [x] | `docs/specs/v0.8.145-boundary-fill-v2.md` | Boundary Fill v2 preview/confirm workflow is implemented with result diagnostics, segment collector, sampled curve boundaries, endpoint-to-endpoint and endpoint-to-segment gap bridging, dedicated Gap HUD editing and ignored-entity diagnostics. |
...
Boundary Fill v1 is available as a bridge between current solid-fill support and future hatch entities. It detects the visible linear face containing a picked point and creates a new filled closed `PolylineEntity`. BF v2 is complete for the current filled-polyline workflow: the tool previews the detected boundary, confirms with Enter/right-click, uses sampled curve boundaries, supports editable small-gap tolerance for endpoint-to-endpoint and endpoint-to-segment gaps, and reports ignored unsupported entities. Holes/islands should wait for a real `HatchEntity`, because subtractive inner loops do not fit the current single-polyline fill model.
