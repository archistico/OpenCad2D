...
## 2026-06-20 — Boundary Fill v2 final documentation reconciliation

Boundary Fill v2 documentation, roadmap and manual checklist now explicitly distinguish endpoint-to-endpoint gaps from endpoint-to-segment gaps. The current completed contract is: normal BFILL HUD shows only seed coordinates; `G`/`Gap` opens a dedicated HUD sub-prompt that shows only the `Gap` textbox after `TAB`; preview/confirm is used before creating the filled polyline; curve boundaries are sampled; small endpoint-to-endpoint and endpoint-to-segment gaps are closed by explicit synthetic `GapBridge` segments without moving source geometry.
