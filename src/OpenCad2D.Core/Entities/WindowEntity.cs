...
using OpenCad2D.Core.Architecture.Windows;
...
/// <summary>
/// Parametric 2D architectural window entity.
/// </summary>
public sealed class WindowEntity : CadEntity
{
    public WindowEntity(
        Point2D insertionPoint,
        double width,
        double wallThickness,
        double frameOffset = 4.0,
            ...
            drawOrder)
    {
        ...
        ValidatePositive(frameOffset, nameof(frameOffset));

        if (frameOffset > wallThickness / 2.0)
        {
            throw new ArgumentOutOfRangeException(
                nameof(frameOffset),
                "Window frame offset cannot be greater than half the wall thickness.");
        }
        ...
        {
            throw new ArgumentException("Window axes cannot be collinear.");
        }
    ...
    public double FrameOffset { get; }

    public AnchorPoint Anchor { get; }

    /// <summary>
    /// Gets whether the window draws a non-destructive wall-opening mask before its visible linework.
    /// </summary>
    ...
    public override EntityKind Kind => EntityKind.Window;

    public WindowGeometry GetGeneratedGeometry()
    {
        return WindowGeometryBuilder.Build(this);
    }
    ...
    public override CadEntity Transform(Matrix2D matrix)
    {
        ...
        {
            throw new InvalidOperationException("Window transform collapsed the entity axes.");
        }
    ...
    public WindowEntity WithParameters(
        Point2D? insertionPoint = null,
        double? width = null,
        double? wallThickness = null,
        double? frameOffset = null,
    ...
    private static void ValidatePositive(double value, string paramName)
    {
        if (value <= 0.0 || double.IsNaN(value) || double.IsInfinity(value))
        {
            throw new ArgumentOutOfRangeException(
                paramName,
                "Window numeric parameters must be finite positive values.");
        }
    }
    ...
    private static Vector2D NormalizeAxis(Vector2D axis, string paramName)
    {
        if (Tolerance.IsZero(axis.Length))
        {
            throw new ArgumentOutOfRangeException(
                paramName,
                "Window axis cannot be a zero-length vector.");
        }
